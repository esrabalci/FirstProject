...
//Prevent the account from being deleted if it's child opportunity

  // Get All the account with child oops with below query
...
List<Account> accWithOpps = [
    ...
    for (Account each : accWithOpps) {
      ...
      //  Trigger.oldMap.get(each.Id) --> We get the value that enter the trigger
      //You cannot use addError method
      //To any record outside the context of Trigger.New or Old MAp